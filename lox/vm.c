...
static Value clockNative(int argCount, Value* args) {
  return OBJ_VAL(copyString("Not implemented", strlen("Not implemented")));
}

#define CREDITS_TEXT "Thanks to Robert Nystrom for the lox language and the great book\n`Crafting Interpreters`, the POSIX-UEFI project from bzt for the\nmalloc(), free() and realloc() functions that used in this project,\nanonymous for the strcmp(), strcpy() and a lot of string\nfunctions, and also GNU-EFI & EDK2 for the UEFI interface."

static Value credits(int argCount, Value* args) {
  return OBJ_VAL(copyString(CREDITS_TEXT, strlen(CREDITS_TEXT)));
}

#define COPYRIGHT_TEXT "As you can see, I used a lot of open source project.\nFor the core of the language itself, it's written by Robert Nystrom, that is released under MIT.\nFor the standard lib I used, some of them are written by myself, others are copied from\nbzt's POSIX-UEFI project and string functions authored by anonymous.\nFor the UEFI Interface I used, they're mostly from EDK2 and GNU-EFI.\nThis project is open source too, you can see it under\ngithub.com/xiaohuangren1001/lox-efi"

static Value copyright(int argCount, Value* args) {
  return OBJ_VAL(copyString(COPYRIGHT_TEXT, strlen(COPYRIGHT_TEXT)));
}

#define CHANGED_TEXT "What's are changed:\n    1.Removed floating-point number support to be fit with UEFI Firmware.\n    2.Removed the functionality to run directly from file, it means we can only use Lox REPL.\n    3.Removed the outputs to stderr, because I do not support this stream. They're now all redirected to stdout."

static Value changes(int argCount, Value* args) {
  return OBJ_VAL(copyString(CHANGED_TEXT, strlen(CHANGED_TEXT)));
}
...
//< Calls and Functions define-native-clock
  defineNative("credits", credits);
  defineNative("copyright", copyright);
  defineNative("changes", changes);
}
